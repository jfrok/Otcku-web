    ...
    <div class="content-2">
        <div class="recent-payments">
            ...
            <table>
                ...
                $sqlUser =  "SELECT * FROM series ORDER BY ID desc LIMIT 8";
                ...
                <td>' . $row['SR_NAME'] . '</td>
                <td>' . $row['ADD_DATE'] . '</td>
                <td><a href="../inc/seriesw.php?search=' . $row["SR_NAME"] . '&searchBtn=" class="btn">View</a></td>
                ';
                    // if ($publisher === $row["USER_ID"]) {
                    //     if ($row["ID"] === $row["ID"]) {

                    echo '<td><form method="POST"><button typy="submit" name="edit-anime" value="' . $row["ID"] . '" class="btn">Edit</button><form></td>
                  <td><form method="POST"><button typy="submit" name="delete-anime" value="' . $row["ID"] . '" class="btn">Delete</button><form></td>';
                    //                     } else {
                    //                         echo '
