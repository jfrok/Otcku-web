...
<div class="content">
    ...
    $moviesName = $database->prepare("SELECT * FROM tvshow");
    $moviesName->execute();
    if ($moviesName->rowCount() > 0) {
        $rows  = $moviesName->fetchAll();
        $html = '<select name="sid" id="sid">';
        foreach ($rows as $row) {
            $html = $html . '<option value="' . $row['ID'] . '">' . $row['TV_NAME'] . '</option>';
        };
        $html = $html . '</select>';
    }
...
-->
                ...
                <br>
                <button class="btn" type="submit" name="upload-tv-ep">Upload</button>
                <br>
            </form>
        </div>
    </div>
    <div class="content-2">
        <div class="recent-payments">
            ...
            <table>
                ...
                //SELECT s.TV_NAME,e.NAME,e.ID,e.IMAGEURL,e.RATE,e.ADD_DATE,e.SID,u.UR_NAME FROM episodetv_info e JOIN tvshow s ON e.SID = s.ID JOIN users u ON u.ID = e.USER_ID ORDER BY e.ID desc LIMIT 10
                $sqlUser =  "SELECT s.TV_NAME,e.NAME,e.ID,e.IMAGEURL,e.RATE,e.ADD_DATE,e.EP_USER_ID,e.SID,u.UR_NAME FROM episodetv_info e JOIN tvshow s ON e.SID = s.ID JOIN users u ON u.ID = e.EP_USER_ID ORDER BY e.ID desc LIMIT 10";
         ...
         <td>' . $row['TV_NAME'] . '</td>
                ...
                <td>' . $row['ADD_DATE'] . '</td>
                <td><a href="../inc/ws.php?search=' . $row["ID"] . '&searchBtn=" class="btn">View</a></td>
                ';
                    if ($publisher === $row["EP_USER_ID"]) {
                        if ($row["ID"] === $row["ID"]) {

                            echo '<td><form method="POST"><button typy="submit" name="edit-series" value="' . $row["ID"] . '" class="btn">Edit</button><form></td>
                      <td><form method="POST"><button typy="submit" name="delete-series-ep" value="' . $row["ID"] . '" class="btn">Delete</button><form></td>';
                        } else {
                            echo '
