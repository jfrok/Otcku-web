    ...
    <div class="content-2">
        <div class="recent-payments">
            ...
            <table>
                ...
                // SELECT s.TV_NAME,e.NAME,e.ID,e.IMAGEURL,e.RATE,e.ADD_DATE,e.SID,EP_USER_ID FROM episodetv_info e INNER JOIN tvshow s ON e.SID = s.ID ORDER BY e.ID desc LIMIT 10

                $sqlUser =  "SELECT s.SR_NAME,e.NAME,e.ID,e.IMAGEURL,e.RATE,e.SID,e.USER_ID,u.UR_NAME,e.ADD_DATE FROM episode_info e JOIN series s ON e.SID = s.ID JOIN users u ON e.USER_ID = u.ID ORDER BY e.ID desc LIMIT 10";
         ...
         <td>' . $row['SR_NAME'] . '</td>
                ...
                <td>' . $row['ADD_DATE'] . '</td>
                <td><a href="../inc/we.php?search=' . $row["ID"] . '&searchBtn=" class="btn">View</a></td>
                ';
