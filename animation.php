...
$numberOfResults = $database->prepare("SELECT * FROM Disny ");
...
for($count = 1; $count<= $totalPages; ++$count){
    if($page == $count){
     // echo '
    //  <div class="pagenum1">
    //  <a style="color:red;"  href="movies.php?page='.$count.'">'.$count.'</a>
    //  </div> ';
    }else{
      // echo '
      // <div class="pagenum">
      // <a  href="movies.php?page='.$count.'">'.$count.'</a>
      // </div> 
      // ';
    }
  // echo'</div> 
...
$results = $database->prepare("SELECT * FROM Disny LIMIT " . $resultsPerPage . " OFFSET " . ($page-1)*$resultsPerPage);
$results->execute();
...
<div class="mimge">
  ...
  <div class="flm">    
  <a href="./db/trilarA.php?search='.$result['ID']. '&searchBtn="</a>  <img src='.$result['IMAGEURL'] .'>    
